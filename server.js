// Hey Bori — Mobile bilingual ES-first chat (pinned input, Clear + Copy)
...
res.setHeader(
"Location",
"https://" + FORCE_DOMAIN + (reqUrl.pathname || "/") + (reqUrl.search || "")
);
return true;
}
return false;
...
"</style>" +
'<section class=screen><header class=hdr><h1 class=t>Hey Bori</h1><p class=s>Haz tu pregunta — Español primero, luego Inglés</p></header>' +
'<div class=chat><iframe src="/inner?lang=' +
encodeURIComponent(lang) +
'" title="Hey Bori Chat" loading=eager></iframe></div></section>'
);
}
...
"document.addEventListener('keydown',function(e){if(e.key==='Escape'){localStorage.removeItem(KEY);render(true);alert('Chat cleared ✅')}});" +
"render(true);" +
"<\\/script>";
...
const systemPrompt =
lang === "en"
? "Respond ONLY in English. Use the full conversation context. Be concise and helpful. End with “— Bori Labs LLC — Let’s Go Pa’lante 🏀”."
...
console.log("✅ Hey Bori bilingual mobile listening on " + PORT)
);
