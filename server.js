// Hey Bori — Mobile-Only Chat (clean version without mic or status bar)
...
return (
'frame-ancestors ' +
(list.length ? list : ['https://heybori.co', 'https://chat.heybori.co']).join(' ')
)
}
...
res.setHeader(
'Location',
'https://' + FORCE_DOMAIN + (reqUrl.pathname || '/')
)
return true
}
return false
...
ua = ua.toLowerCase()
return /iphone|ipod|ipad|android|mobile|silk|opera mini|blackberry|bb10|windows phone/.test(
ua
)
}
...
return resolve('Missing API key. — Bori Labs LLC — Let’s Go Pa’lante 🏀')
...
'<div class=chat><iframe src="/inner" title="Hey Bori Chat" loading=eager></iframe></div></section>'

const INNER_HTML =
'<!doctype html><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1">' +
'<title>Hey Bori Chat</title><style>:root{--line:#e6e6e6;--user:#eef4ff;--assistant:#f7f7f7}' +
'html,body{margin:0;height:100%;background:#fff;font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#111}' +
...
'</style><main><div id=messages></div><form id=ask-form autocomplete=off>' +
'<textarea id=q placeholder="Haz tu pregunta… / Ask your question…" required></textarea><button id=send type=submit>Send</button></form></main>' +
'<script>' +
...
? html(res, MOBILE_SHELL)
: text(res, 200, 'Mobile only')
...
'0'
if (!rateOK(ip))
return json(res, 429, { answer: 'Rate limit — try again later.' })
let body = ''
...
role: 'system',
content:
'ES/EN reply (Spanish first, then English). Use context. End with “— Bori Labs LLC — Let’s Go Pa’lante 🏀”.'
},
...hist,
{ role: 'user', content: q }
]
...
console.log('✅ Hey Bori (clean mobile version) listening on ' + PORT)
)
